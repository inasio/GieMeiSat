function gms_stationary_plot_back
...
T_range = [0,100];
Step_x = ceil(N/200);
L_up = linspace(L_init,L_end,200);
L_down = [linspace(L_end,L_init,100)];
H = 1/(N+1);
X = (H:H:1-H)';
D = 1;
Eps = 0.02;
...
for I = 2:length(L_up)
  [1,I/length(L_up)]
  [T,W] = ode15s(@rhs, T_range, Y(:,I-1),[],L_up(I));
...
figure(1)
surf(X(3:Step_x:N),L_up,U(3:Step_x:N,:)')
rotate3d on
view(10,50)
shading interp
...
for I=1:4
    nom = ['~/UBC/Chamba_Michael/patterns_in_growing_domains/code/auto/gms/branch',num2str(I)];
    nom = load(nom);
    figure(2)
    semilogx(nom(:,1),nom(:,4),'r')
    hold on
    semilogx(L_up,max(V),'--')
end

fid = fopen(['up_GMS_N_',num2str(N),'.txt'],'w');
maxV = max(V);
for I = 1:length(L_up)
...
Y = [W(end,1:N)';W(end,N+1:2*N)'];
for I = 2:length(L_down)
  [1,1-I/length(L_down)]
  [T,W] = ode15s(@rhs, T_range, Y(:,I-1),[],L_down(I));
  Y(:,I) = [W(end,1:N)';W(end,N+1:2*N)'];
end
...
figure(3)
surf(X(3:Step_x:N),L_down,U(3:Step_x:N,:)')
rotate3d on
view(10,50)
shading interp
...
for I=1:4
    ...
    semilogx(nom(:,1),nom(:,4),'r')
    hold on
    semilogx(L_down,max(V),'--')
end

fid = fopen(['down_GMS_N_',num2str(N),'.txt'],'w');
maxV = max(V);
for I = 1:length(L_down)
  fprintf(fid,['%%6.6f  %6.6f\n'],...
    L_down(I),maxV(I));
end
fclose(fid)
